...
from apps.backend.database.parsers.csv_parser import parse_csv
from apps.backend.database.parsers.xml_parser import parse_xml
from apps.backend.database.parsers.xlsx_parser import parse_xlsx
...
@app.post("/measurements/import", response_model=MeasurementIngestResponse, status_code=201)
async def import_measurement_file(
    ...
    try:
        return _ingest_parsed_measurement(db, parsed)
    except Exception as exc:
        db.rollback()
        raise HTTPException(500, f"Ошибка сохранения в БД: {exc}")

def _ingest_parsed_measurement(db: Session, parsed: ParsedMeasurement) -> dict:
    """Внутренняя функция сохранения распарсенных данных в БД"""
    
    device = db.scalar(select(models.Device).where(models.Device.serial_number == parsed.device_serial))
    if device is None:
        device_type = db.scalar(select(models.DeviceType).where(models.DeviceType.code == parsed.device_type_code))
        if device_type is None:
            device_type = models.DeviceType(code=parsed.device_type_code, name=parsed.device_type_code)
            db.add(device_type)
            db.flush()
        device = models.Device(device_type_id=device_type.id, serial_number=parsed.device_serial, name=parsed.device_serial)
        db.add(device)
        db.flush()

    sensor_by_position: dict[int, models.Sensor] = {}
    for s in parsed.sensors:
        sensor = db.scalar(select(models.Sensor).where(models.Sensor.device_id == device.id, models.Sensor.position == s.position))
        if sensor is None:
    ...
    if parsed.measurement_object:
        measurement_object = db.scalar(select(models.MeasurementObject).where(func.lower(models.MeasurementObject.name) == parsed.measurement_object.lower()))
    ...
    group = db.scalar(select(models.MeasurementGroup).where(models.MeasurementGroup.group_key == group_key))
    group_is_new = group is None
    if group is None:
        group = models.MeasurementGroup(name=parsed.measurement_object or parsed.measurement_name, group_key=group_key)
        db.add(group)
        db.flush()
